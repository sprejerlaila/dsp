...
>> Frequentists think statistics as a repetead random sample of data. Therefore probability is the number of outcomes if we were to repeat the experiment infinite times with some fix parameters.
Bayesian on the other hand measures the probability of a certain outcome given a fixed sample data. Therefore, it depends on the prior beliefs and likelihood of sample data 
